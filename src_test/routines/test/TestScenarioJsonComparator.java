...
public class TestScenarioJsonComparator extends TalendFakeJob {
	...
	public void setupInput() throws Exception {
		...
		JsonDocument doc = new JsonDocument(testFile);
	...
	public void calculateConflicts() throws Exception {
		...
		ArrayNode conflictsArrayNode = null;
		ArrayNode arrayNodeNewROStatusNavi = null;
		ArrayNode arrayNodeNewROStatusCore = null;
		ArrayNode arrayNodeNewProductODStatusNavi = null;
		ArrayNode arrayNodeNewProductODStatusCore = null;
		try {
			...
			conflictsArrayNode = resultNode.withArray("conflicts");
			globalMap.put("conflicts_node", conflictsArrayNode);
			arrayNodeNewROStatusNavi = resultNode.withArray("new_rightsownership_process_status_navi");
			arrayNodeNewROStatusCore = resultNode.withArray("new_rightsownership_process_status_core");
			arrayNodeNewProductODStatusNavi = resultNode.withArray("new_product_operational_data_status_navi");
			arrayNodeNewProductODStatusCore = resultNode.withArray("new_product_operational_data_status_core");
								...
								for (JsonNode nodeProduct2RO : nodeProduct2ROArray) {
									...
									if (TimestampUtil.isOverlapping(refValidFrom, refValidTo, compValidFrom, compValidTo)) {
										...
										if (inConflict) {
											...
											// we must use the businesspartner of RO of product 2 !
											ObjectNode bpNode1 = (ObjectNode) nodeProduct2RO.get("businesspartner");
											if (bpNode1 == null) {
												throw new Exception("Rightownership: " + nodeProduct2RO.get("rights_ownership_id") + ": no businesspartner found. Product-ID: " + nodeProduct2.get("product_id"));
											}
											ObjectNode disputeParty1 = bpNode1.deepCopy();
											...
											// we must use the businesspartner of the RO of product 1 !
											ObjectNode bpNode2 = (ObjectNode) nodeProduct1RO.get("businesspartner");
											if (bpNode2 == null) {
												throw new Exception("Rightownership: " + nodeProduct1RO.get("rights_ownership_id") + ": no businesspartner found. Product-ID: " + nodeProduct1.get("product_id"));
											}
											ObjectNode disputeParty2 = bpNode2.deepCopy();
			...
			globalMap.put("tJava_3_ERROR_MESSAGE", ex.getMessage());
			log.error("Calculate failed: ", ex);
			log.error("Document:\n" + doc.getJsonString(true, false));
			throw ex;
		}
