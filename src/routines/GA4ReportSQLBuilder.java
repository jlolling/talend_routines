...
public class GA4ReportSQLBuilder {
	...
	private String reportDimensionFilters = null;
	private String reportMetricFilters = null;
	private static Map<Integer, List<CalculatedColumn>> mapCalculatedColumns = new HashMap<>();
	
	public String getReportDimensionFilters() {
		return reportDimensionFilters;
	}

	public void setReportDimensionFilters(String reportFilters) {
		this.reportDimensionFilters = reportFilters;
	}

	public String getReportMetricFilters() {
		return reportMetricFilters;
	}

	public void setReportMetricFilters(String reportFilters) {
		this.reportMetricFilters = reportFilters;
	}
	...
	public String buildReportView() {
		...
		sb.append("create or replace view v_ga4_report_" + StringUtil.camelToSnake(reportName) + " as\n");
		sb.append("select\n");
		...
		for (Entry e : allMetrics) {
			...
			if (firstLoop) {
				firstMetric = e;
				firstLoop = false;
				sb.append("\nfrom ga4_report_result_metric_values_" + reportId + " m");
				sb.append(e.id);
			} else {
				sb.append("\njoin ga4_report_result_metric_values_" + reportId + " m");
				sb.append(e.id);
				// on row_num
				sb.append(" on m");
				sb.append(e.id);
		...
		for (Entry e : allDimensions) {
			...
			sb.append("\nleft join ga4_report_result_dimension_values_" + reportId + " d");
			sb.append(e.id);
			sb.append(" on d");
			sb.append(e.id);
			sb.append(".row_num = m");
	...
	public String buildReportTable() {
		...
		StringBuilder sb = new StringBuilder();
		String tableName = "ga4_report_data_" + StringUtil.camelToSnake(reportName);
		sb.append("create table if not exists " + tableName + " (\n");
		sb.append("report_date datetime not null,\n");
		sb.append("property_id bigint not null,\n");
	...
	public String buildReportDataTableSourceView() {
		...
		StringBuilder sb = new StringBuilder();
		String tableName = "ga4_report_data_" + StringUtil.camelToSnake(reportName);
		String reportView = "v_ga4_report_" + StringUtil.camelToSnake(reportName);
		sb.append("create or replace view  v_" + tableName + "_source as\n");
		sb.append("select\n");
		sb.append("v.report_date,\n");
		sb.append("v.property_id,\n");
		sb.append("v.row_num");
		for (Entry e : allDimensions) {
			if (isDimensionInReport(e.apiName) == false) {
				continue;
		...
		index = 1;
		for (Entry e : dynParams) {
			sb.append("\njoin ga4_report_result_dyn_param_values_");
			sb.append(reportId);
			sb.append(" d" + index);
			sb.append(" on d" + index + ".property_id = v.property_id and d" + index + ".report_date = v.report_date and d" + index + ".row_num = v.row_num and d" + index + ".dyn_param_name = '" + e.columnName + "'");
			index++;
		}
		sb.append(";\n");
		return sb.toString();
	...
	public List<Entry> getDimensionsFromDynReports() {
		...
		List<String> names = getPlaceHolderNames(reportDimensionFilters);
		...
		names = getPlaceHolderNames(reportMetricFilters);
		for (String name : names) {
			Entry e = new Entry();
			e.apiName = name;
			e.columnName = name;
